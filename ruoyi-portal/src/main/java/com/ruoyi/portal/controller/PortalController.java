...
{
    private static final String prefix = "portal/theme";
    ...
    private static Cache<String,Object> portalCache= CacheUtil.newTimedCache(1000*60*60*3);

    private String getTheme(){
        return configService.selectConfigByKey(CmsConstants.KEY_PORTAL_THEME);
    }
    /**
     * 首页
     *
    ...
    public String index(Model model) {
        model.addAttribute("pageUrl", "portal/index");
        ...
        System.out.printf(prefix+"/" + getTheme() + "/index");
    ...
    public String Individual_initiation()
    {
        return prefix+"/" + getTheme() + "/perReginstration";
    }
    /**
     * 企业入会
     * @return
    ...
    public String Corporate_membership()
    {
        return prefix+"/" + getTheme() + "/enterpriseMemb";
    }
    /**
     * 工会概况
     * @return
    ...
    {
        return prefix+"/" + getTheme() + "/ghGeneralSituation";
    }
     ...
     * @return
     */
    @GetMapping("/situationCon")
    public String General_Situation_Con()
    {
        return prefix+"/" + getTheme() + "/ghGeneralSituationCon";
    }
    /**
     * 登录
     * @return
    ...
    {
        return prefix+"/" + getTheme() + "/editPassword";
    }
    /**
     * 文件下载
     * @return
     */
    @GetMapping("/fileDowns")
    public String File_Down(Model model)
    {
...
//         form.setAuditState(CmsConstants.AUDIT_STATE_AGREE.toString());
         startPage();
         List<Attachment> resources = attachmentService.selectAttachmentList(form);
        ...
        return prefix+"/" + getTheme() + "/fileDown";
    }
    /**
     * 更多
     * @return
    ...
    public String More(HttpServletRequest request,Model model)
    {
    	String type = request.getParameter("type");
    	 startPage();  
    	List<Article> resources = new ArrayList<Article>();
    	if("1".equals(type)){
    		resources=articleService.selectArticlesByArticleRegionType(ArticleRegionType.REGION_NEW);
    	}else if("2".equals(type)){
    		resources=articleService.selectArticlesByArticleRegionType(ArticleRegionType.REGION_HOT);
    	}else if("3".equals(type)){
    		resources=articleService.selectArticlesByArticleRegionType(ArticleRegionType.REGION_Dynamic1);
    	}else if("4".equals(type)){
    		resources=articleService.selectArticlesByArticleRegionType(ArticleRegionType.REGION_Dynamic2);
    	}else{
    		resources=articleService.selectArticleList(new Article());
    	}
        model.addAttribute("categoryId", "resource");
     //   Attachment form = new Attachment();
//         form.setStatus(CmsConstants.STATUS_NORMAL);
...
//        form.setAuditState(CmsConstants.AUDIT_STATE_AGREE.toString());
    ...
    public String tag(@PathVariable("tagId") String tagId, Model model) {
        ...
        Article form = new Article();
        form.setTag(tagId);
        model.addAttribute("pageUrl", "portal/tag/" + tagId);
        startPage();
        List<Article> articles = articleService.selectArticleList(form);
        PageInfo pageInfo=new PageInfo(articles);
        model.addAttribute("total", pageInfo.getTotal());
...
//        }
    ...
    public String nav(Model model) {
        ...
        List<LinkType> linkTypeList=(List<LinkType>)portalCache.get(KEY_LINK_TYPE_LIST);
        if(CollectionUtil.isEmpty(linkTypeList)){
            LinkType form=new LinkType();
            form.setStatus(CmsConstants.STATUS_NORMAL);
            linkTypeList=linkTypeService.selectLinkTypeList(form);
            portalCache.put(KEY_LINK_TYPE_LIST,linkTypeList);
        }
        for(LinkType type:linkTypeList){
            List<Link> tempList=(List<Link>)portalCache.get(KEY_LINK_LIST+type.getLinkType());
            if(CollectionUtil.isEmpty(tempList)){
                ...
                portalCache.put(KEY_LINK_LIST+type.getLinkType(),tempList);
            }
    ...
    @GetMapping("/portalTheme")
    public String portalTheme(Model model){
        PortalTheme form = new PortalTheme();
        startPage();
        List<PortalTheme> themes = portalThemeService.selectPortalThemeList(form);
        ...
        String currentTheme = portalThemeService.queryCurrentPortalTheme();
