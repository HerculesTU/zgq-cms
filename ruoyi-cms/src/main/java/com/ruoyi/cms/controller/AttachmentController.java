...
public class AttachmentController extends BaseController
{
    ...
    public AjaxResult upload(@RequestParam("file") MultipartFile file) throws Exception {
        ...
        String url = "";
        if (4 != CloudConstant.CloudService.FASTDFS.getValue()) {
        ...
        attachment.setFileUrl(url.replaceAll(";", ""));
        attachment.setSize(file.getSize());
        attachment.setFileType(suffix);
        attachment.setFileName(fileName);
        attachment.setCreateBy(ShiroUtils.getLoginName());
        attachment.setCreateTime(new Date());
        attachment.setUserId(String.valueOf(ShiroUtils.getSysUser().getUserId()));
        attachment.setSort(attachmentService.getMaxSort() + 1);
        return toAjax(attachmentService.insertAttachment(attachment));
    }
