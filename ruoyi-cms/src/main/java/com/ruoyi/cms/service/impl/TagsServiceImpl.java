...
public class TagsServiceImpl implements ITagsService 
{
    ...
    @Override
    public List<Tags> selectPortalTabs() {
//        String s=(String)EhCacheUtils.getDefaultInfo(CmsConstants.KEY_TAGS_PORTAL);
        String s=null;
        List<Tags> tags=null;
        if(StringUtils.isNotEmpty(s)){
            tags = JSONArray.parseArray(s, Tags.class);
        }else{
            tags=tagsMapper.selectPortalTabs();
            EhCacheUtils.putDefaultInfo(CmsConstants.KEY_TAGS_PORTAL,tags);
        }
        return tags;
    }
}
