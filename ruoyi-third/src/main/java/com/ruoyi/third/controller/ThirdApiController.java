...
import com.ruoyi.third.service.impl.HrSmsCallBackImpl;
...
public class ThirdApiController extends BaseController
{
    ...
    @ResponseBody
    public AjaxResult sendSms(String phone, String prefix, String content) {
        HrSmsCallBackImpl hrSmsCallBack = SpringUtils.getBean("hrSmsCallBack");
        Map map = new HashMap<String, Object>();
        map.put("phone", phone);
        map.put("prefix", prefix);
        map.put("content", content);
        hrSmsCallBack.setParams(map);
        AjaxResult result = SmsUtil.sendHRSms(phone, content, hrSmsCallBack);
        return result;
    }
